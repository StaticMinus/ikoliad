...
export const BehindTheLensSection: React.FC = () => {
  ...
  const standardPosts = posts.filter((p) => p.type === 'standard' && p.id !== featuredPost?.id);

  return (
    <section className="w-full bg-white py-[60px] px-[20px] border-b border-gray-100 selection:bg-black selection:text-white">
      <div className="max-w-[1200px] mx-auto">
        
        {/* ── Section Header ───────────────────────────────── */}
        <div className="mb-[48px]">
          {/* Small Grey Blog Badge */}
          ...
          {/* Subtitle and View All Posts Button */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <a
              href="#all-posts"
              className="inline-flex items-center justify-center bg-black text-white rounded-[40px] text-[14px] font-semibold px-7 py-3 transition-transform duration-200 hover:scale-[1.02] shadow-sm w-fit font-sans shrink-0"
            >
              View all posts
            </a>
          </div>
        ...
        {featuredPost && (
          <div className="grid grid-cols-1 lg:grid-cols-2 rounded-[20px] border border-[#f0f0f0] bg-[#fcfcfc] min-h-[520px] overflow-hidden mb-[40px] shadow-xs group">
            
            {/* Left Side: Interactive Video Container */}
            <div className="relative w-full h-[320px] lg:h-full min-h-[320px] overflow-hidden bg-black cursor-pointer">
              <video
                autoPlay
                loop
                muted
              ...
              {/* 4 L-Shaped Corner Brackets (12px length, 1.5px border, 15px inset) */}
